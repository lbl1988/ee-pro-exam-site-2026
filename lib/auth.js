...
export async function verifyPassword(pwd, stored) {
  try {
    ...
    const [salt, iterStr, hashB64] = parts;
    if (!salt || !iterStr || !hashB64) return false;
...
export function clearCookieHeader(name) {
  return name + '=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=Lax; Secure';
}
